...
$(function () {
  ...
  $('#shareQuestionnaireForm').on('submit', function (e) {
    ...
    formData.push({ name: 'mahasiswa_ids[]', value: JSON.stringify(selectedIds) }); // Tambahkan ID mahasiswa yang dipilih ke dalam array

    // AJAX request to submit the form data
    $.ajax({
      url: '/pembagian/share', // URL to your share method in the controller
      type: 'POST',
      data: formData,
  ...
  $('#shareQuestionnaireModal').on('show.bs.modal', function () {
    ...
    $.ajax({
      ...
      type: 'GET',
      success: function (response) {
        console.log(response); // Cek respons di console
        ...
        response.data.forEach(function (kuesioner) { // Gunakan response.data
          questionnaireSelect.append(`<option value="${kuesioner.id}">${kuesioner.judul}</option>`); // Pastikan 'judul' ada dalam respons
        });
      },
      error: function (xhr) {
